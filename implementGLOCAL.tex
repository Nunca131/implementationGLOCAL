...
\NR{We have implemented both general alignment algorithms for partially local
alignments with additive gap costs, starting from Eq.~(\ref{eq:maxrec}) and
Eq.~(\ref{eq:maxrec2}). In the practical implementation of Eq.~(\ref{eq:maxrec2})} we omit the the
...
Eq.~(\ref{eq:maxrec}) only considers the Hasse diagram of the partial
order $\prec$, its transitive closure is required for
Eq.~(\ref{eq:maxrec2}). On the other hand, Eq.~(\ref{eq:maxrec2}) provides
...
\NR{The code is available on \texttt{GitHub} (\TODO{LINK}).}
...
explicitly constructs the partial orders \NR{$\pprec$ for Eq.~(\ref{eq:maxrec})
and $\prec$ for Eq.~(\ref{eq:maxrec2})} and uses this information
...
$s=|A_0\setminus A^*|$ + \NR{$|$}$A^*\setminus A_0$\NR{$|$} for the number sequences that
...
resulting in an upper bound of $O(2^N n^N h^2)$ effort. The effort can
  be reduced to $O(2^N n^N h N)$ by implementing Eq.~(\ref{eq:maxrec})
  instead of Eq.~(\ref{eq:maxrec2}) \NR{as shown in Fig.~(\ref{fig:manyLoc})}. 
