...
\abstract{Multiple sequence alignments are a crucial intermediate step in a
  ...
  heuristic approximations, \PFS{exact pairwise alignments are readily
...
scored. The notion of ``local multiple alignments'' \PFS{appears mostly in
...
\TODO{R3: ``some more references covering the last 5 years for increasing
  attractivity''}
...
resulting in an upper bound of $O(2^N n^N h^2)$ effort. \PFS{The effort can
  be reduced to $O(2^N n^N h N)$ by implementing Equ.~(\ref{eq:maxrec})
  instead of Equ.~(\ref{eq:maxrec2}). Both variants are available in the
...
which we tagged for local instead of global alignment.  Fig.~\ref{fig:locs}
...
\TODO{Fig.~\ref{fig:manyLoc} also compares the resource consumption for
  Eqns.~(\ref{eq:maxrec}) and (\ref{eq:maxrec2}). We observe that ***** }
Much more interesting \PFS{than the comparisons of running times, however,
...
\PFS{Global alignments can be constructed from arbitrarily dissimilar
  sequences. This is not the case for local alignments, however: if no
  sufficiently similar substrings can be found, the sequences remain
  unaligned. It is possible therefore to construct filters loss-less
  filters 
