...
\TODO{R3: \\
1.some more references covering the last 5 years for increasing attractivity\\
``Notice that 33 references are cited and most of them do not add value to the manuscript in terms of timliness.\\
2. check
...
\abstract{Multiple sequence alignments are a crucial intermediate step in a
  ...
  heuristic approximations, pairwise alignments are typically
  computed by exact dynamic programming algorithms \TODO{pairwise alignments are typically
  computed by exact dynamic programming algorithms -- true? R1 claims it is not. 
  R1 suggests ``if one wants to compute exact alignments, then dynamic programmin is 
  the tool}. In the pairwise case,
...
\TODO{R1: ``usually gapless`` is not true: if you do not allow gaps, then 
there is no point to use DP and quadratic algorithms. When you use DP it means
that you allow insertions and deletions.} \NR{I think R1 missed the point in 
this sentence.}

Local variants of sequence alignment, on the one hand, play an important role
...
step for cleaning initial alignments before the release.  Both
  benchmark sets are intended for global multiple alignments. We therefore
  inspected a subset of the reference alignments and manually inspected
  overhanging ends, which we tagged for local instead of global alignment.
  Fig.~\ref{fig:locs} summarizes the distribution of local ends in
