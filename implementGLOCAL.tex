...
(for arbitrary numbers of input sequences\NR{ $X$}) are typically NP-hard
\citep{Kececioglu:93,Wang:94,Bonizzoni:01,Just:01,Manthey:03,Elias:06}, they
are solved by heuristic approximation algorithms, see \citet{Baichoo:17}
for a recent review.

As the exact 3-way and 4-way alignments have increased in usage,
...
rearrangments by simultaneously comparing the rearranged region \NR{to} both of
...
deletions of a prefix or a suffix are \NR{penalized}, or as local, allowing the
omission of prefixes or suffixes at not cost. We will briefly outline the
theoretical results in the following section. \NR{While t}he presentation by
\citet{Retzlaff:18a} is purely theoretical and did not supply a reference
implementation\NR{, t}he present contribution closes this gap.
...
scoring \NR{(\emph{memoization})} table $S$ holds the optimal alignments of prefixes. The only
...
sequences\NR{ since all \emph{inactive} can be derived by $X\setminus\{A \cup D\}$ }.
As the alignment progresses, an \textit{inactive} sequence may
...
denote this relation by $\pprec$. \PFS{The initial condition is $A=A_0$,
  where $A_0$ is the set of left-global sequences, and $D=\emptyset$.}
...
additive score. \PFS{The special case that a sequence $k$ that is both
  left- and right-local remains completely unaligned is handled by a
  directed transition from \textit{inactive} to \textit{dead} restricted to
  $I_k=0$, see \citet{Retzlaff:18a} for details.}
...
\PFS{The backtracing recursion is a rather straightforwards generalization
  of the backtracing scheme for the Smith-Waterman algorithm. The first
  step is to find the optimal score partially local alignment. Denote by
  $(A^*,D^*)$ that unique maximal state w.r.t.\ $preceq$, i.e., $A^*$ is
  the set of all right-global sequences and $D^*$ is the set of all
  right-local sequences. Hence $A^*\cup D^*$ contains all input sequences.
  The optimal score is the maximum over all multi-indices $I$ with
  constraint that $I_k=n_k$, the length of the $k$-th sequence, for all
  $k\in A^*$, with the the maximum taken over all indices $I_k$ with
  $k\in D^*$.  The maximum value of $I_k$ determines the right boundary of
  the right-local sequence.  The recursion then proceeds, as usual, to find
  the index or state transition in Equ.(\ref{eq:maxrec}). The backtracing
  recursion terminates as soon as all left-global variables $k\in A_0$ have
  reached the left end oft the sequences, i.e., $I_k=0$ for all $k\in A_0$.
  The left boundary of a left-local sequences $l\notin A_0$ equals the
  index $I_l$ at this point.}

\section{\uppercase{Implementation}}

We have implemented a general alignment algorithm for partially local
alignments with additive gap costs.
\NR{One major divergence from the previously given theory is that during
the recursion steps we only allow state changes if characters are aligned.
This means particularly for the start of a earlier \emph{inactive} sequence
that it can only be started with the alignment of one of its characters. 
There are immediate consequences to this way of implementation: (1) 
$F = \{D\}$, a set of final states that always contain the right-global sequences as \emph{active} 
sets but also any variation of right-local sequences as either \emph{inactive} 
or \emph{active} since it has to be possible to end the alignment with any
right-local sequence and it is impossible to transfer them from 
\emph{active} to \emph{dead} after the alignment is done; note $(A^*,D^*) \in F$.
A similar problem arises throughout backtracing and reaching possible 
initial states, hence $A_0 = \{A\}$ respectively. 
(2) It has to be possible to not only start not only aligning one local 
sequence but any number of them. Hence the Hasse graph still contains the 
same amount of nodes but the number of edges increases drastically with 
respect to the number of semi-global and local sequences.}
\TODO{verschieben?}


\SAFTWARE{} is written in \TODO{\texttt{Java} and uses whatever
  ... libraries}.  In principle, the software \TODO{name} is capable of
accepting an arbitrary number of input sequences. However, the resource
requirements quickly become prohibitive.
...
\TODO{number of entries in the Hasse diagram?} 

Let $g$=$|A_0\cap A^*|$ be the number of global sequences, $s=|A_0\setminus
A^*|$ + $A^*\setminus A_0$ the number sequences that are local and one end
and global at the other, and $\ell$ the number of local sequences. Then
there are 
\begin{equation}
  h = 1^a 2^s 3^{\ell} 
\end{equation} 
...
sequences \NR{can} pass through all three states. All combinations of these
